...
El máximo puntaje Z corresponde a 2.23. Esto significa que el hijo o hija más alto se encuentra $\pm$ desviaciones estándar sobre el promedio
...
Pero ahora, a qué valor observado corresponden el puntaje Z mínimo y máximo? para esto, recurriremos a una lógica de ecuación. Utilizaremos la fórmula del puntaje Z y despejaremos el valor observado.

En esta fórmula, el resultado es el puntaje Z, pero ahora necesitamos que nuestro resultado sea el valor observado, entonces, para llegar a esto, debemos **dividir** el puntaje Z por la desviación estándar y sumarle la media.

```{r}
(max * desv_estandar) + media
```

Con ello, ahora podemos decir que el hijo o hija más alto mide 73.7 pulgadas.

Apliquemos la misma lógica para calcular el valor observado mínimo.

```{r}
(min * desv_estandar) + media
```

La altura mínima entre todos los hijos es de 61.7 pulgadas.
...
En promedio, los hijos e hijas presentes en la base miden 68.10 pulgadas, y su altura varía entre $\pm$ 2.58 pulgadas respecto del promedio. Entonces, podríamos decir que el 68% de los hijos e hijas miden entre 65.52 y 70.68 pulgadas. En la misma línea, la estatura de el 95% de hijos e hijas se encuentra entre 62.94 y 73.26
