...
# Retomemos el ejercicio anterior
...
- `|>` es el **pipe nativo de R**. Cumple la misma función general que `%>%`: permite encadenar pasos y leer el código como **“y luego…”**.

- Cuando escribimos `dplyr::filter()`, la forma `paquete::función()` indica de qué paquete proviene la función. Por eso no necesitamos ejecutar `library(dplyr)` antes.

>  **Importante:** el paquete sí debe estar instalado en el computador. Si no lo está, primero debemos instalarlo con `install.packages("dplyr")`
:::
...
El escenario planteado por $H_0$ **se utiliza como referencia y evaluamos si los datos entregan evidencia suficiente para rechazarlo.**.
...
t
\approx
\frac{\text{diferencia observada entre las medias}}
{\text{error estándar de esa diferencia(incertidumbre)}}
$$
...
La pregunta busca saber si existe una **diferencia**, pero no establece de antemano cuál de los dos grupos debería tener un promedio mayor. Por lo tanto, nos interesa detectar una diferencia en **cualquiera de las dos direcciones**. Por eso utilizamos una **prueba *t* de dos colas**.
...
En R, especificamos que nuestra prueba es de **dos colas** mediante:
...
### ¿Qué diferencia observamos? {.unnumbered}
...
::: {.callout-note}
## ¿Qué son los grados de libertad? {.unnumbered}
La columna `df` muestra los **grados de libertad** de la prueba. Podemos pensarlos como una medida de **cuánta información tiene la prueba para estimar la variabilidad**. En una prueba *t* de Welch, **R los calcula automáticamente**.
:::


### ¿Qué nos dice el valor *p*? {.unnumbered}
...
::: {.callout-note}
## Nivel de significación{.unnumbered}
...
### Veamos esta evidencia gráficamente {.unnumbered}
...
### ¿Qué nos dice el intervalo de confianza? {.unnumbered}
...
Estos resultados son **distintas piezas de información relacionadas entre sí.**.

Cuando la prueba está correctamente especificada, estos resultados deben ser coherentes entre sí respecto de la decisión sobre $H_0$. Si aparecen contradicciones entre ellos, conviene revisar las especificaciones de la prueba o los cálculos realizados.
...
### ¿Qué observamos en la muestra? {.unnumbered}
...
### ¿Qué podemos decir sobre la población? {.unnumbered}
...
nos interesaba cualquier diferencia:

- que las mujeres **sin hijos** tuvieran un ingreso promedio mayor;
- o que las mujeres **con hijos** tuvieran un ingreso promedio mayor.

En cambio, ahora nuestra pregunta es:

> **¿Las mujeres sin hijos tienen un ingreso laboral promedio mayor que las mujeres con hijos?**

Aquí ya no nos sirve cualquier diferencia.  
Nos interesa una diferencia **en una dirección específica**:
...
::: {.callout-important}
Cuando preguntamos si dos grupos son **distintos**, la diferencia puede aparecer en cualquiera de las dos direcciones.

Cuando preguntamos si un grupo es **mayor que otro**, estamos planteando una dirección específica.
:::
...
> **No basta con que *t* sea positivo: debe ser significativamente mayor que cero.**
...
### Lo que proviene de los datos {.unnumbered}
...
### Lo que depende de nuestra pregunta {.unnumbered}
...
### Preparemos los datos  {.unnumbered}
...
# Síntesis del práctico

En este práctico trabajamos con la **prueba *t* para comparar dos medias** y vimos cómo la pregunta de investigación determina la forma en que planteamos la prueba.

A lo largo del práctico aprendimos que:

- la prueba *t* compara una diferencia observada con la incertidumbre asociada a esa diferencia;
- el signo de *t* indica la **dirección de la diferencia** entre los grupos;
- el valor *p* nos ayuda a evaluar qué tan compatible es el resultado observado con el escenario planteado por \(H_0\);
- el intervalo de confianza nos muestra un rango de valores compatibles con los datos;
- en una prueba de **dos colas** nos interesa detectar diferencias en cualquiera de las dos direcciones;
- en una prueba de **una cola** nos interesa evaluar una dirección específica;
- la dirección de la prueba debe definirse **antes de observar los resultados**, a partir de la pregunta y de \(H_A\).

### ¿Cómo elegimos `alternative`? {.unnumbered}
